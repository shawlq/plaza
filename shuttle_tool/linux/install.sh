#!/usr/bin/env bash
# 安装 shuttle 到 PREFIX：按 HTTP(S) 地址克隆到 linux/.shuttle.env/repo，并写入 config
set -euo pipefail
...
echo "=== shuttle 安装：将克隆远程仓库到 ${CLONE_DIR} 并写入 ${CONFIG_PATH}（勿提交到 Git）==="
echo ""
...
"${maybe_sudo[@]}" chown -R "$(id -un):$(id -gn)" "${CONFIG_DIR}"
